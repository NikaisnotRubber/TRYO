## 畢業論文項目《DETR架構的監控場景下抽菸行為檢測模型》
開發了一個基於輕量化DETR的實時吸煙行為檢測系統，專為低質量監控視頻和邊緣設備進行優化，相較於YOLOv8s，mAP95高出5.6%
- 端到端Transformer模型：構建了一個基於DETR的檢測系統，直接生成預測框，消除了傳統的NMS後處理，提升推理速度（T4 GPU延遲從3.4ms減少到2.9ms）。
- 編碼器優化：將自注意力（self-attention）替換為PSA注意力，降低訓練和推理成本，同時保持性能表現。
- 輔助檢測頭：設計了一個一對多樣本匹配輔助檢測頭，*以在訓練過程中提供額外的預測框提供監督，但不增加推理開銷*。
- 對比降噪訓練：通過*學習非樣本數據的特徵增強區分能力*，減少了在擁擠場景、小目標和噪聲環境中的錯誤。
- 損失函數：引入基於Wasserstein距離的損失函數，通過利用分布距離而非IoU指標，改進了小目標檢測效果。
該研究優異的表現（在抽菸數據集上相比 YOLOv8s，mAP 提升 5.6%，T4 顯卡下 FPS 從 294 提升至 329，
然而參數量僅小幅增加（11.2M -> 13.6M），*榮獲計算機學院本科優秀畢業論文（成績於年級前二十）*

## 主要更動點在
* 在"/ultralytics/nn/modules/head.pyTRYODetect" 實現了模型的檢測頭（實現了DETR的檢測頭架構（一對多的輔助檢測頭部、特徵投影、對比去噪模塊、嵌入位置編碼、Query Selection、Transformer模塊）

整理簡歷時順便修改了一下模型，有鑑於原有的頸部網路設計類似於YOLO11（只在最深層特徵圖使用PSA注意力），所以從原有的backbone和neck更改為YOLO11的版本，這樣可以更充分運用官方提供的預訓練權重，未來能用到的話會更容易能fine-tune出模型。
